var passwordEntryDiv = document.getElementById("password-entry");
var coloredBoxDiv = document.getElementById("colored-box");
var passwordInput = document.getElementById("password-input");
var startButton = document.getElementById("start-button");

var password;
var key;
var lastSecond;

startButton.onclick = function () {
    password = passwordInput.value.toUpperCase();
    password = password.replace(" ", "0");
    password = password.replace("1", "I");
    password = password.replace("8", "B");
    password = password.replace("9", "G");
    password = password.replace("0", "O");
    if (password.length > 0) {
        key = base32tohex(password);
        passwordEntryDiv.style = "display:none";
        coloredBoxDiv.style = "display:block";
        setInterval(generateOtpAndShowColor, 50);
    }
};

function generateOtpAndShowColor() {
    var epoch = Math.round(new Date().getTime() / 1000.0);
    var second = Math.round(epoch);
    if (second === lastSecond) return;
    lastSecond = second;
    var time = leftpad(dec2hex(Math.floor(epoch / 5)), 16, '0');

    var shaObj = new jsSHA("SHA-1", "HEX");
    shaObj.setHMACKey(time, "HEX");
    shaObj.update(time);
    var hmac = shaObj.getHMAC("HEX");

    var offset = hex2dec(hmac.substring(hmac.length - 1));
    var otp = (hex2dec(hmac.substr(offset * 2, 8)) & hex2dec('7fffffff')) + '';
    otp = parseInt(otp.substr(otp.length - 6, 6), 10);

    var r = otp / 10000 * 255 / 100;
    var g = otp / 100 % 100 * 255 / 100;
    var b = otp % 100 * 255 / 100;

    if (epoch - Math.floor(epoch / 5) * 5 < 1) {
        coloredBoxDiv.style = `display:block;background: rgb(${r},${g},${b})`;
    } else {
        coloredBoxDiv.style = "";
    }
}

function dec2hex(s) { return (s < 15.5 ? '0' : '') + Math.round(s).toString(16); }
function hex2dec(s) { return parseInt(s, 16); }

function base32tohex(base32) {
    var base32chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567";
    var bits = "";
    var hex = "";

    for (var i = 0; i < base32.length; i++) {
        var val = base32chars.indexOf(base32.charAt(i).toUpperCase());
        bits += leftpad(val.toString(2), 5, '0');
    }

    for (i = 0; i + 4 <= bits.length; i += 4) {
        var chunk = bits.substr(i, 4);
        hex = hex + parseInt(chunk, 2).toString(16);
    }
    return hex;

}

function leftpad(str, len, pad) {
    if (len + 1 >= str.length) {
        str = Array(len + 1 - str.length).join(pad) + str;
    } else {
        str = str.substring(0, len);
    }
    return str;
}
